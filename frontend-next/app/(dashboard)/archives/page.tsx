...
export default function ArchivesPage() {
  ...
  const [activeTab, setActiveTab] = useState<'archives' | 'reports'>('reports');
    ...
    try {
      ...
      if (report.approval_items && report.approval_items.length > 0) {
        const approvalRows = [
          ...
          ...report.approval_items.map((item, i) => [
            i + 1,
            item.category || '',
            item.subject || '',
            item.sender || '',
  ...
  const getCategoryChartData = () => {
    ...
    return selectedReport.categories.filter(c => c.count > 0).map(c => ({
      name: `${c.code} ${c.name}`,
      value: c.count,
    }));
  };
        ...
        <div className="flex gap-5">
          <div className="flex-1 space-y-5">
            ...
            {reports.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {reports.map((r) => (
                  <div key={r.id} onClick={() => viewReportDetail(r.id)} className={`bg-white rounded-2xl border p-5 cursor-pointer transition-all hover:shadow-md ${selectedReport && selectedReport.period?.includes(r.report_date) ? 'border-purple-400 ring-1 ring-purple-200' : 'border-slate-200'}`}>
          ...
          {selectedReport && (
            <div className="w-[500px] shrink-0">
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden sticky top-20 animate-fadeIn max-h-[calc(100vh-6rem)] overflow-y-auto">
                <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-6 sticky top-0 z-10 shadow-lg">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-xl font-bold">📊 {selectedReport.type_label} 리포트</h3>
                    <button onClick={() => setSelectedReport(null)} className="text-white/80 hover:text-white transition text-2xl">&times;</button>
                  </div>
                  <p className="text-sm text-white/90">{selectedReport.period}</p>
                </div>
                  ...
                  {reportLoading ? (
                    ...
                    <>
                      ...
                      <button
                        ...
                        className="w-full px-4 py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl font-semibold hover:from-green-600 hover:to-emerald-600 transition disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
                          <div className="text-2xl font-bold text-blue-700">{selectedReport.overview.period_emails}</div>
                          <div className="text-xs text-blue-600 mt-1">기간 내 이메일</div>
                        </div>
                        <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-center">
                          <div className="text-2xl font-bold text-red-700">{selectedReport.overview.approval_needed}</div>
                          <div className="text-xs text-red-600 mt-1">승인 필요</div>
                        </div>
                        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-center">
                          <div className="text-2xl font-bold text-yellow-700">{selectedReport.priorities.high}</div>
                          <div className="text-xs text-yellow-600 mt-1">긴급 이메일</div>
                        </div>
                        <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
                          <div className="text-2xl font-bold text-green-700">{selectedReport.overview.key_emails_count}</div>
                          <div className="text-xs text-green-600 mt-1">주요 이메일</div>
                        </div>
                      </div>

                      {/* 카테고리별 통계 */}
                      <div>
                        <h4 className="font-bold text-slate-900 mb-3 text-sm">카테고리별 분포</h4>
                          ...
                          {selectedReport.categories.filter(c => c.count > 0).map((cat) => {
                            ...
                            return (
                              <div key={cat.code} className="flex items-center gap-3">
                                <div className="w-20 text-xs text-slate-600 font-semibold">{cat.code} {cat.name}</div>
                                ...
                                <div className="w-12 text-xs text-slate-500 text-right">{percentage}%</div>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      {/* 우선순위 분포 */}
                      <div>
                        <h4 className="font-bold text-slate-900 mb-3 text-sm">우선순위 분포</h4>
                        <div className="flex gap-2">
                          <div className="flex-1 bg-red-50 border border-red-200 rounded-lg p-3 text-center">
                            <div className="text-lg font-bold text-red-700">{selectedReport.priorities.high}</div>
                          ...
                          <div className="flex-1 bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-center">
                            <div className="text-lg font-bold text-yellow-700">{selectedReport.priorities.medium}</div>
                            <div className="text-xs text-yellow-600">일반</div>
                          </div>
                          <div className="flex-1 bg-slate-50 border border-slate-200 rounded-lg p-3 text-center">
                            <div className="text-lg font-bold text-slate-700">{selectedReport.priorities.low}</div>
                            <div className="text-xs text-slate-600">낮음</div>
                          </div>
                        </div>
                      </div>
                      ...
                      {selectedReport.approval_items && selectedReport.approval_items.length > 0 && (
                        <div>
                          <h4 className="font-bold text-slate-900 mb-3 text-sm">승인 필요 항목 ({selectedReport.approval_items.length}건)</h4>
                          <div className="space-y-2 max-h-64 overflow-y-auto">
                            {selectedReport.approval_items.slice(0, 10).map((item, idx) => (
                              <div key={idx} className="bg-red-50 border border-red-200 rounded-lg p-3">
                                ...
                                <div className="text-xs text-slate-600 mt-1 truncate">{item.sender} | {item.company}</div>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {selectedReport.key_emails && selectedReport.key_emails.length > 0 && (
                        <div>
                          <h4 className="font-bold text-slate-900 mb-3 text-sm">주요 이메일 ({selectedReport.key_emails.length}건)</h4>
                          <div className="space-y-2 max-h-64 overflow-y-auto">
                            {selectedReport.key_emails.slice(0, 10).map((item, idx) => (
