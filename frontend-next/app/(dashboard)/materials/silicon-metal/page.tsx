...
export default function SiliconMetalPage() {
  const [data, setData] = useState<SiliconData | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError('');
      try {
        const res = await fetch(apiUrl('/api/v1/commodity-prices/silicon-metal'));
        if (res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="animate-fadeIn">
        <div className="flex items-center gap-2 text-sm text-slate-500 mb-1">
          ...
          <span className="text-slate-700 font-medium">메탈 실리콘</span>
        </div>
        <h1 className="text-2xl font-bold text-slate-900">메탈 실리콘 시세</h1>
        <p className="text-sm text-slate-500 mt-1">산업용 실리콘 (Si Metal #441) · GFEX 선물 · CNY/톤</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 animate-fadeInUp">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="text-xs text-slate-500 font-medium uppercase tracking-wide mb-1">Si-Metal · GFEX</div>
            {loading ? (
      ...
      {data && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fadeInUp delay-2">
          ...
          {data.futures_price && (
            <div className="bg-white rounded-2xl border border-slate-200/80 p-5">
              <div className="text-xs text-slate-500 font-medium mb-1">GFEX 선물</div>
