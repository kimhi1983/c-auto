...
export default function NaphthaPage() {
  ...
  const fetchData = async (selectedRange: string) => {
    ...
    setError('');
    try {
      const res = await fetch(apiUrl(`/api/v1/commodity-prices/naphtha?range=${selectedRange}&interval=1d`));
      if (res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="animate-fadeIn">
        <div className="flex items-center gap-2 text-sm text-slate-500 mb-1">
          ...
          <span className="text-slate-700 font-medium">납사</span>
        </div>
        <h1 className="text-2xl font-bold text-slate-900">납사 (Naphtha) 시세</h1>
        <p className="text-sm text-slate-500 mt-1">Brent Crude Oil 선물 기준 · ICE · USD/배럴</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 animate-fadeInUp">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="text-xs text-slate-500 font-medium uppercase tracking-wide mb-1">BZ=F · ICE</div>
            {loading ? (
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.prices}>
                <defs>
                  <linearGradient id="colorNaphtha" x1="0" y1="0" x2="0" y2="1">
