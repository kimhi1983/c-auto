...
export default function SalesInputPage() {
  ...
  useEffect(() => {
    ...
    const loadWorkflow = async () => {
      try {
        ...
        // 비고에서 출고방식 추출
        if (wf.note?.startsWith('출고방식: ')) setShipMethod(wf.note.replace('출고방식: ', ''));
        // 배송정보 로드
        if (wf.deliveryAddress) setDeliveryAddress(wf.deliveryAddress);
        if (wf.deliveryContact) setDeliveryContact(wf.deliveryContact);
        if (wf.deliveryPhone) setDeliveryPhone(wf.deliveryPhone);
        // 품목
        const items = wf.items || JSON.parse(wf.itemsData || '[]');
        if (items.length > 0) {
          const loadedRows: SaleRow[] = items.map((item: any) => ({
            id: genId(),
  ...
  useEffect(() => {
    ...
    custDebounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(
          apiUrl(`/api/v1/erp/customers/search?q=${encodeURIComponent(custSearch)}&limit=10`),
          { headers: authHeaders() }
        );
        const json = await res.json();
        if (json.status === 'success') {
  ...
  const handleWorkflowSave = async (action: 'draft' | 'submit') => {
    ...
    try {
      const payload = {
        ...
        note: shipMethod ? `출고방식: ${shipMethod}` : undefined,
      ...
      if (json.status === 'success') {
        ...
        if (action === 'submit') {
          ...
          // 편집 모드 URL 파라미터 제거
          if (isEdit) window.history.replaceState({}, '', '/erp/sales');
        }
      } else {
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-card overflow-hidden">
        ...
        {editingWorkflowId && (
          <div className="px-5 py-3 bg-blue-50 border-b border-blue-200 flex items-center justify-between">
            ...
            <a href="/erp/sales" className="text-xs text-blue-600 hover:text-blue-800 font-medium">새 문서 작성</a>
          </div>
        )}
        ...
        <div className="px-5 py-4 border-b border-slate-200">
          ...
          {/* 배송정보 */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-2 mt-3 pt-3 border-t border-slate-200">
            <div className="flex items-center gap-2">
              <label className="text-sm text-slate-500 w-16 shrink-0">도착주소</label>
              <input
                type="text"
                value={deliveryAddress}
                onChange={e => setDeliveryAddress(e.target.value)}
                placeholder="배송 도착 주소"
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                placeholder="도착지 담당자명"
                className={`${headerInputCls} flex-1`}
              />
            </div>
            <div className="flex items-center gap-2">
              <label className="text-sm text-slate-500 w-16 shrink-0">연락처</label>
              <input
                type="text"
                value={deliveryPhone}
