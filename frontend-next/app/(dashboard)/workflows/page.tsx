  ...
  if (selected) {
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        ...
        {/* 배송정보 */}
        {(selected.deliveryAddress || selected.deliveryContact || selected.deliveryPhone) && (
          <div className="bg-white rounded-2xl border border-slate-200/80 p-6">
            <h3 className="text-sm font-bold text-slate-900 mb-3">배송정보</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {selected.deliveryAddress && (
                <div>
                  <span className="text-xs text-slate-500">도착주소</span>
                  <p className="text-sm font-medium text-slate-800 mt-0.5">{selected.deliveryAddress}</p>
                </div>
              )}
              {selected.deliveryContact && (
                <div>
                  <span className="text-xs text-slate-500">담당자</span>
                  <p className="text-sm font-medium text-slate-800 mt-0.5">{selected.deliveryContact}</p>
                </div>
              )}
              {selected.deliveryPhone && (
                <div>
                  <span className="text-xs text-slate-500">연락처</span>
                  <p className="text-sm font-medium text-slate-800 mt-0.5">{selected.deliveryPhone}</p>
                </div>
              )}
            </div>
          </div>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a
                      href={`/erp/${w.workflowType === 'SALES' ? 'sales' : 'purchases'}?edit=${w.id}`}
                      onClick={e => e.stopPropagation()}
                      className="p-1 rounded-lg text-slate-300 hover:text-blue-600 hover:bg-blue-50 transition"
                      title="수정"
                    >
